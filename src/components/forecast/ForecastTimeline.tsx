...
export function ForecastTimeline({ hours, timestamp, onSelect }: { hours: HourlyForecastViewModel[]; timestamp: string; onSelect: (timestamp: string) => void }) {
  return <div className="forecast-timeline" aria-label="Previsioni orarie">{hours.map(hour => <button className="timeline-card" key={hour.timestamp} data-score-tone={scoreTone(hour.score.finalScore)} data-selected={hour.timestamp === timestamp} aria-pressed={hour.timestamp === timestamp} onClick={() => onSelect(hour.timestamp)}>
