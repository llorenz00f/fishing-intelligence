...
export function SessionCard({ session, compact = false }: { session: JournalSession; compact?: boolean }) {
  ...
  return <Link className="session-card" data-compact={compact} href={`/sessions/${session.id}`}>
    <div className="session-card-head"><time dateTime={session.startTime}>{new Intl.DateTimeFormat("it-IT", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(session.startTime))}</time><span className="session-score" data-score-tone={scoreTone(session.conditionScore)}>{session.conditionScore ?? "N/D"}<small>SCORE</small></span></div>
